...
:::note
The delegated nameservers are not necessarily a different DNS provider. They can be Cloudflare nameservers, in the same account or a different one.
:::
...
The `A` record for `ns1.sub.example.com` is glue. It is shadowed (the `sub.example.com` delegation takes effect first), but it serves a valid purpose: the parent zone publishes it in the additional section of the referral, alongside the `NS` delegation, so resolvers can bootstrap the subdomain lookup.
